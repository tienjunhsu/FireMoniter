...
collection_test = mongo_client['fire_moniter']['daily_moniter']
...
def sub_strategy(strategy_name):
    today = datetime.date.today().strftime('%Y-%m-%d')
    trade_days = data_db['trade_day'].find({'date': {'$lte': today}}, {'_id': False}).sort('date', -1).limit(2)
    trade_days = list(trade_days)
    if trade_days[0]['date'] < today:
...
@app.route('/rt_pct/<strategy_name>')
def rt_pct(strategy_name):
    today = datetime.date.today().strftime('%Y-%m-%d')
    trade_days = data_db['trade_day'].find({'date': {'$lte': today}}, {'_id': False}).sort('date', -1).limit(2)
    trade_days = list(trade_days)
    if trade_days[0]['date'] < today:
    ...
    print(strategy_name)
    cursor = collection.find({'date': current_trade_day},
                             {'_id': 0, 'time_stamp': 1, 'sz50': 1, 'hs300': 1, 'zz500': 1, strategy_name: 1,
                              'ih_spread': 1, 'if_spread': 1, 'ic_spread': 1}).sort('time_stamp')
    response_data = {}
    ...
    i = 0
    strategy_list = strategies + [strategy_name]
    ...
    for name in index_names:
        ...
        if df is not None and len(df) > 0:
            json_item['data'] = df[['time_stamp', name]].dropna().values.tolist()
        response_data['data'].append(json_item)
    time_range_item = {'name': '-', 'data': [[min_time, -0.0000], [max_time, 0.0000]], 'yAxis': 1,
                       'lineWidth': 1}  # 用于调整时间轴
    response_data['data'].append(time_range_item)
...
def tick():
    ...
    if isTrading():
        ...
        response_data['data'] = [datetime.datetime.now().strftime('%Y-%m-%d %H:%M'), random.random(), random.random(),
...
if __name__ == '__main__':
    app.run(host='192.168.2.112', port=5001, debug=True)
