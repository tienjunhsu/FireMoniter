...
while real_total < totalAmount:
    ...
    real_total = (df.position_num * df.close).sum()
df.to_excel(u'华泰吴学敏计划持仓.xlsx', index=False)
filename = u'吴雪敏.xls'
now_df = pd.read_csv(filename,sep='\t',skiprows=3,encoding='gbk')
now_df = now_df[[u'="证券代码"',u'="证券名称"',u'="证券数量"',u'="最新市值"']]
...
writer = ExcelWriter(u'华泰吴学敏加减仓.xlsx')
